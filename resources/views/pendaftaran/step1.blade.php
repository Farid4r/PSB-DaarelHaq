            ...
            <form action="{{ route('register.step1.post') }}" method="POST" class="space-y-10">
                ...
                <div>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Nomor Ijazah <span class="text-xs text-gray-400 font-normal">(Kosongkan jika belum ada / lulusan baru)</span></label>
                            <input type="text" name="nomor_ijazah" value="{{ old('nomor_ijazah', $registration->nomor_ijazah ?? '') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-primary focus:border-primary @error('nomor_ijazah') border-red-500 @enderror"placeholder="Contoh: DN-01/D-SMP/13/0000001">
                            @error('nomor_ijazah')<span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                            @enderror
                        </div>
